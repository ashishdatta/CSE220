...
// End While                                                        --
//--------------------------------------------------------------------------------------------------------------
void cK1as::Assemble(unsigned const pPass) {
	mCurrAddr = 0;
	string token = mLex.Token();
	while (token != "") {
		if (token[0] == '.') {
			if (pPass == 1) {
			...
			if (pPass == 1) {
				...
				string mnemonic = mLex.Token();
				cInstr *instr = AssembleInstr(mnemonic);	
			...
			if (pPass == 1) {
				...
				cInstr *instr = AssembleInstr(token);
				mTextSeg.AddInstr(*instr);
				delete instr;
			}
			mCurrAddr++;
...
//--------------------------------------------------------------------------------------------------------------
void cK1as::AssembleDirective(string const& pDirective) {
	string strAddr = mLex.Token();
	char *end;
	tAddress address = static_cast<tAddress>(strtol(strAddr.c_str(), &end, 10));
	if (pDirective == ".DATA") {
		mDataSeg.Address(address);
	}
	else {
		mTextSeg.Address(address);
	}
	mCurrAddr = address;
}
...
// End If
//--------------------------------------------------------------------------------------------------------------
cInstr* cK1as::AssembleInstr(string const& pMnemonic) {
	if (pMnemonic == "ADD" || pMnemonic == "NEG" || pMnemonic == "NOR" || pMnemonic == "POP" || pMnemonic == "PUSH" || pMnemonic == "ROL") {
		return AssembleInstrTypeR(pMnemonic);
	}
	else if (pMnemonic == "BEQ" || pMnemonic == "BLT" || pMnemonic == "BR" || pMnemonic == "BSUB") {
		return AssembleInstrTypeB(pMnemonic);
	}
	else if (pMnemonic == "HALT" || pMnemonic == "RET") {
		return AssembleInstrTypeN(pMnemonic);
	}
	else if (pMnemonic == "IN" || pMnemonic == "OUT" || pMnemonic == "LDI") {
		return AssembleInstrTypeRI(pMnemonic);
	}
	else {
		return AssembleInstrTypeRV(pMnemonic);
...
//--------------------------------------------------------------------------------------------------------------
cInstr* cK1as::AssembleInstrTypeB(string const& pMnemonic) {
	string strLabel = mLex.Token();
	cLabel branchTargetLabel = mTextSeg.Label(strLabel);
	cInstr *B = new cInstrTypeB(pMnemonic, branchTargetLabel);
	return B;
}
...
//--------------------------------------------------------------------------------------------------------------
cInstr* cK1as::AssembleInstrTypeN(string const& pMnemonic) {
	cInstr *N = new cInstr(pMnemonic);
	return N;
}
...
//--------------------------------------------------------------------------------------------------------------
cInstr* cK1as::AssembleInstrTypeR(string const& pMnemonic) {
	string strReg = mLex.Token();
	cRegister reg(strReg);
	cInstr *R = new cInstrTypeR(pMnemonic, reg);
	return R;
}
...
//--------------------------------------------------------------------------------------------------------------
cInstr* cK1as::AssembleInstrTypeRI(string const& pMnemonic) {
	string strReg = mLex.Token();
	string strInt = mLex.Token();
	cRegister reg(strReg);
	cInteger integer(strInt);
	cInstr *RI = new cInstrTypeRI(pMnemonic, reg, integer);
	return RI;
}
...
//--------------------------------------------------------------------------------------------------------------
cInstr* cK1as::AssembleInstrTypeRV(string const& pMnemonic) {
	string strReg;
	string strVar;
	...
	cVariable var = mDataSeg.Variable(strVar);
	cRegister reg = cRegister(strReg);
	cInstr *RV = new cInstrTypeRV(pMnemonic, reg, var);
	return RV;
}
...
//--------------------------------------------------------------------------------------------------------------
void cK1as::AssembleLabel(string const& pName) {
	cLabel newLabel(pName, mCurrAddr);
...
//--------------------------------------------------------------------------------------------------------------
void cK1as::AssembleVariable(string const& pName) {
	string strInitValue = mLex.Token();
	char *end;
	tWord initValue = static_cast<tWord>(strtol(strInitValue.c_str(), &end, 10));
	cVariable newVar(pName, mCurrAddr, initValue);
	mDataSeg.Add(newVar);
}
//--------------------------------------------------------------------------------------------------------------
// Run()
...
void cK1as::Run() {
	...
	mLex.Reset();
	Assemble(2);
	cBinary bin(mArg.BinFname());
	bin.Write(mDataSeg, mTextSeg);
}

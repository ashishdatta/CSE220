...
//--------------------------------------------------------------------------------------------------------------
cTextSegment::cTextSegment(tAddress const pAddress) :
	cSegment(pAddress),
	mInstrs(),
	mLabels()
{
...
//--------------------------------------------------------------------------------------------------------------
cTextSegment::cTextSegment(cTextSegment const& pTextSegment) :
	cSegment(pTextSegment),
...
//--------------------------------------------------------------------------------------------------------------
void cTextSegment::AddInstr(cInstr& pInstr) {
	pInstr.Encode();
...
//--------------------------------------------------------------------------------------------------------------
void cTextSegment::AddLabel(cLabel const& pLabel) {
	mLabels[pLabel.Name()] = pLabel;
}
...
//--------------------------------------------------------------------------------------------------------------
tByte const *cTextSegment::Contents() const {
	tWord *contents = new tWord[mInstrs.size()];
	vector<cInstr>::const_iterator it = mInstrs.begin();
	for (unsigned offset = 0; it != mInstrs.end(); ++it, ++offset) {
		tWord encoding = it->Encoding();
		memcpy(&contents[offset], &encoding, sizeof(tWord));
	}
...
//--------------------------------------------------------------------------------------------------------------
void cTextSegment::Copy(cTextSegment const& pTextSegment) {
	cSegment::Copy(pTextSegment);
	mInstrs = pTextSegment.mInstrs;
	mLabels = pTextSegment.mLabels;
}
...
// mLabels[pName]
//--------------------------------------------------------------------------------------------------------------
cLabel cTextSegment::Label(string const& pName) {
...
//--------------------------------------------------------------------------------------------------------------
tUint32 cTextSegment::Size() const {
	return 9 + 4 * mInstrs.size();
}

//--------------------------------------------------------------------------------------------------------------
// Overload operator=
//--------------------------------------------------------------------------------------------------------------
cTextSegment& cTextSegment::operator=(cTextSegment const& pTextSegment) {
